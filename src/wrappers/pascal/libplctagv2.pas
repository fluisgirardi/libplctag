...
const
  ...
  function plc_tag_set_raw_bytes(tag:cint32; offset:cint; block:PUInt8; blocksize:cuint):cint; cdecl; external LibPLCTagLibName;
  //LIB_EXPORT int plc_tag_get_raw_bytes(int32_t id, int offset, uint8_t *buffer, int buffersize);
  function plc_tag_get_raw_bytes(tag:cint32; offset:cint; buffer:PUInt8; buffersize:cint):cint; cdecl; external LibPLCTagLibName;

  // string accessors

  //LIB_EXPORT int plc_tag_get_string(int32_t tag_id, int string_start_offset, char *buffer, int buffer_length);
  function plc_tag_get_string(tag_id:cint32; string_start_offset:cint; buffer:PChar; buffer_length:cint):cint; cdecl; external LibPLCTagLibName;
  //LIB_EXPORT int plc_tag_set_string(int32_t tag_id, int string_start_offset, const char *string_val);
  function plc_tag_set_string(tag_id:cint32; string_start_offset:cint; const string_val:PChar):cint; cdecl; external LibPLCTagLibName;
  //LIB_EXPORT int plc_tag_get_string_length(int32_t tag_id, int string_start_offset);
  function plc_tag_get_string_length(tag_id:cint32; string_start_offset:cint):cint; cdecl; external LibPLCTagLibName;
  //LIB_EXPORT int plc_tag_get_string_capacity(int32_t tag_id, int string_start_offset);
  function plc_tag_get_string_capacity(tag_id:cint32; string_start_offset:cint):cint; cdecl; external LibPLCTagLibName;
  //LIB_EXPORT int plc_tag_get_string_total_length(int32_t tag_id, int string_start_offset);
  function plc_tag_get_string_total_length(tag_id:cint32; string_start_offset:cint):cint; cdecl; external LibPLCTagLibName;

  function plc_tag_get_pascal_string(tag_id:cint32; string_start_offset:cint; out aStr:String):cint;
...
var
  ...
  name_str: PChar = nil;
begin
  name_len := plc_tag_get_string_length(tag_id, string_start_offset);
  if (name_len <=0)    then exit(PLCTAG_ERR_TOO_SMALL);
  if (name_len >= 256) then exit(PLCTAG_ERR_TOO_LARGE);

  // create a string for this. */
  name_str := GetMem(name_len + 1);
  try
    if(not Assigned(name_str)) then exit(PLCTAG_ERR_NO_MEM);

    // copy the name
    rc := plc_tag_get_string(tag_id, string_start_offset, name_str, name_len + 1);
    if (rc = PLCTAG_STATUS_OK) then begin
      aStr:=name_str;
    end;
    exit(rc);
  finally
    if Assigned(name_str) then
      Freemem(name_str);
  end;
